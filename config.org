...
Need to fork this package and apply some fixes from forks, and make it
do the right thing when hitting backspace on a top empty list entry.

#+BEGIN_SRC emacs-lisp
  (req-package org-autolist
    :require org
    :config
    (define-activator org-autolist-mode)
    :hook (org-mode . turn-on-org-autolist-mode))
#+END_SRC

...
make sure it saves its state somewhere in this directory.
...
** pretty-symbols
We don't actually use ~pretty-symbols-mode~. Instead, we just take the
appropriate prettification rules defined by it for a given mode and
add them to ~prettify-symbols-alist~ when we set the major mode.
...
#+BEGIN_SRC emacs-lisp
  (use-package pretty-symbols
    ...
    (progn
      (require 'prog-mode)
      (cl-defun prettify-symbols-get-alist-for-mode
          (&optional (mode major-mode)
                     (patterns pretty-symbol-patterns))
        (cl-loop
         with major-mode = mode
         for pattern in patterns
         for keyword = (apply 'pretty-symbol-pattern-to-keyword pattern)
         for regexp = (s-chop-prefix "\\<" (s-chop-suffix "\\>" (car keyword)))
         for char = (nth 3 (cadr (cadr (cadr keyword))))
         if (and regexp char)
         collect (cons regexp char)))

      (cl-defun prettify-symbols-setup-hook ()
        (setq prettify-symbols-alist
              (cl-union prettify-symbols-alist
                        (prettify-symbols-get-alist-for-mode)))))
    (add-hook 'after-change-major-mode-hook 'prettify-symbols-setup-hook)
    :ensure t)
#+END_SRC
...
** ido-completing-read+
This uses the bleeding-edge branch of ido-completing-read+.
...
* el-get package installation & loading
...
#+BEGIN_SRC emacs-lisp
  (defun el-get-do-setup ()
    (interactive)
    (cl-loop for src in el-get-sources
...
** Load the el-get version of ido-cr+
el-get installs the bleeding edge version of ido-cr+, but package.el
installs a possibly older version of ido-cr+ to satisfy dependencies.
To get the el-get version, we need to unload and reload the package.

#+BEGIN_SRC emacs-lisp
  (unload-feature 'ido-completing-read+ t)

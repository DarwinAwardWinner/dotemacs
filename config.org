...
Note that the [[*Bootstrappping][bootstrapping]] code also installs the [[https://github.com/radian-software/straight.el][Straight package
manager]], which it uses to install Org mode. Here we complete the
setup so Straight can be used to manage all pacakges below.
...
This is the file where everything set via =M-x customize= goes. This
needs to be loaded before the package configuration section because
req-package needs to know my Guthub username.
...
Straight to install packages by default. See [[https://github.com/radian-software/straight.el#integration-with-use-package][here]] for more
information.
...
and its dependencies (particularly [[https://github.com/jwiegley/use-package][~use-package~]]).
...
** ass-mode
This is for editing ASS/SSA subtitle files.

#+BEGIN_SRC emacs-lisp
  (req-package ass-mode
    :straight
    (ass-mode
     :type git
     :flavor melpa
     :host github
     :repo "disbeliever/ass-mode"))
#+END_SRC
...
#+END_SRC
*** magit-ido
Needed for ido support in magit.

#+BEGIN_SRC emacs-lisp
  (req-package magit-ido)
#+END_SRC
...
For some reason, calling ~req-package~ on noflet simply does nothing
during init, but it works after init. So we just do it after init. I
...
We use ~without-user-input~ here mainly because sometimes packages (e.g.
...
All code after this point can assume that all the declared packages
are now installed and ready for use (either loaded or with autoloads
fully set up). (Except for noflet, see above.)
* Tweaks
...
#+BEGIN_SRC emacs-lisp
  ...
  immediately."
    (cl-flet ((start-process (name buffer program &rest program-args)
...
Sometimes after filling a long line with ~C-q~, ~auto-fill-mode~, etc.,
  ...
  clipboard for other applications as well."
    ...
    (setq kill-ring
          (cl-loop
           ...
           unless (or
                   (string-match-p "^https?://" kill)
                   (string-match-p (regexp-quote kill) bufstring))
           collect kill))
    (ignore-errors (gui-set-selection 'CLIPBOARD (or (car kill-ring) "")))
    (ignore-errors (gui-set-selection 'PRIMARY ""))
    (ignore-errors (gui-set-selection 'SECONDARY ""))
    (message "Kill ring cleared of HTTP URLs and entries matching buffer %S" (buffer-name buffer)))
#+END_SRC

** Fix bad behavior in org-ctags
See https://emacs.stackexchange.com/a/76352/2046 and
https://lists.gnu.org/archive/html/emacs-orgmode/2023-03/msg00299.html.
...
For some reason, Emacs sometimes starts up with the scratch buffer in
Fundamental mode with no initial message. Until the root cause is
isolated, we just fix this manually afterward

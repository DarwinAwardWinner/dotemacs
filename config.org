...
** TODO keydef
This package seems to be orphaned. It should be replaced with
bind-key. But first all usage must be converted.

#+BEGIN_SRC emacs-lisp
  (req-package keydef :quelpa)
#+END_SRC
...
If this command is run after an Emacs Lisp expression in any buffer,
the expression will be deleted and then replaced with the value that
it evaluates to.

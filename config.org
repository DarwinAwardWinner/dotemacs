...
window. So the whole thing is wrapped in ~save-window-excursion~ to
ensure that the window configuration remains unchanged (but the
compile log buffer is left intact so any warnings or errors can still
be investigated).
...
#+BEGIN_SRC emacs-lisp
  (use-package header2
    ...
    :config (progn
              ...
              (defun header-not-part-of-emacs ()
...
#+BEGIN_SRC emacs-lisp
  (defadvice modify-file-local-variable-prop-line (after cleanup-semicolon activate)
    (save-excursion
      (goto-char (point-min))
...
#+BEGIN_SRC emacs-lisp
  (let ((gdf-path
         (if (file-executable-p "/opt/local/bin/gdf")
             "/opt/local/bin/gdf"
           (executable-find "gdf"))))
    (when gdf-path
      (setq directory-free-space-program gdf-path)))
#+END_SRC

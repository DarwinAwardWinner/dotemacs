...
#+BEGIN_SRC emacs-lisp
  (req-package ess
    ...
    (require 'tramp)
    :config
    ;; Ensure that loading ess-r-package also loads ess-rd
    (eval-after-load 'ess-r-package '(require 'ess-rd))
    ...
    ;; Silently auto-update width in R console buffers
    (defsubst ess-execute-screen-options-invisibly ()
      "Run `ess-execute-screen-options' with INVISIBLY = t"
      (ess-execute-screen-options t))
    (add-hook 'ess-r-post-run-hook #'ess-execute-screen-options-invisibly)
    ;; Tweak jit-lock settings only in R buffers
    (defun ess-r-setup-jit-lock ()
      (setq-local
       jit-lock-defer-time 0.25))
    (add-hook 'ess-r-mode-hook #'ess-r-setup-jit-lock))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (req-package git-gutter
    :config
    (define-advice git-gutter--turn-on
        (:before-until (&rest args) disallow-on-tramp)
      "Don't enable git-gutter in TRAMP buffers."
      ;; We can ignore errors here because if buffer file name is
      ;; invalid, then git gutter won't activate anyway.
...
#+BEGIN_SRC emacs-lisp
  (req-package nextflow-mode
    :straight
    (nextflow-mode
     :type git
     :flavor melpa
     :files ("nextflow-mode.el")
     :host github
...
** Turn off autopair mode in password prompts

#+BEGIN_SRC emacs-lisp
  (defun turn-off-autopair-mode ()
    (autopair-mode -1))
  (add-hook 'read-passwd-mode-hook #'turn-off-autopair-mode)
#+END_SRC

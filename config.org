...
** Don't use ns_selection_fg_color and ns_selection_bg_color
On Mac OS, Emacs uses ns_selection_fg_color and ns_selection_bg_color
to determine the colors for the region face. When Mac OS activates
dark mode, these colors change to dark blue, but they don't change
back when dark mode is deactivated (at least not in Emacs). So we
avoid using these and instead use specific colors.
...
starting the server if it's already running.
...
#+BEGIN_SRC emacs-lisp
  (unless (or noninteractive server-process)
    (ignore-errors
